...
/* let the thread float over the process affinity mask */
static void pin_to_legal() {
  pthread_setaffinity_np(pthread_self(), sizeof(legal_cpus), &legal_cpus);
}
#else
/* macOS has no thread-to-core affinity on Apple silicon. The closest control
 * is the QoS class: USER_INTERACTIVE steers a thread onto performance cores,
 * but the scheduler still picks which one, so -b is effectively a no-op. */
static void init_affinity() {}
